...
namespace SRTS
{
    public static class SRTSArrivalActionUtility
    {
                    ...
                    yield return new FloatMenuOption(label, () =>
               {
                   ...
                   else
                   {
                       if (acceptanceReport.FailMessage.NullOrEmpty())
                           return;
                       Messages.Message(acceptanceReport.FailMessage, new GlobalTargetInfo(destinationTile), MessageTypeDefOf.RejectInput, false);
                   }
               });
            }
        }
        ...
        {
            Func<FloatMenuAcceptanceReport> acceptanceReportGetter1 = () => TransportPodsArrivalAction_AttackSettlement.CanAttack(pods, settlement);
            Func<TransportPodsArrivalAction_AttackSettlement> arrivalActionGetter1 = () => new TransportPodsArrivalAction_AttackSettlement(settlement, PawnsArrivalModeDefOf.EdgeDrop);
            ...
            Func<FloatMenuAcceptanceReport> acceptanceReportGetter2 = () => TransportPodsArrivalAction_AttackSettlement.CanAttack(pods, settlement);
            Func<TransportPodsArrivalAction_AttackSettlement> arrivalActionGetter2 = () => new TransportPodsArrivalAction_AttackSettlement(settlement, PawnsArrivalModeDefOf.CenterDrop);
        ...
        public static IEnumerable<FloatMenuOption> GetGIFTFloatMenuOptions(CompLaunchableSRTS representative, IEnumerable<IThingHolder> pods, Settlement settlement, Caravan car)
        {
            ...
            return SRTSArrivalActionUtility.GetFloatMenuOptions<TransportPodsArrivalAction_GiveGift>(() => TransportPodsArrivalAction_GiveGift.CanGiveGiftTo(pods, settlement), () => new TransportPodsArrivalAction_GiveGift(settlement), "GiveGiftViaTransportPods".Translate(settlement.Faction.Name, FactionGiftUtility.GetGoodwillChange(pods, settlement).ToStringWithSign()), representative, settlement.Tile, car);
        }
        ...
        {
            return SRTSArrivalActionUtility.GetFloatMenuOptions<TransportPodsArrivalAction_VisitSettlement>(() => TransportPodsArrivalAction_VisitSettlement.CanVisit(pods, settlement), () => new TransportPodsArrivalAction_VisitSettlement(settlement), "VisitSettlement".Translate(settlement.Label), representative, settlement.Tile, car);
        }
    }
}